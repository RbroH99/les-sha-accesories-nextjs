...
export function getOptimizedImageUrl(
  ...
  if (transformations) {
    if (transformations.width)
      params.append("tr", `w-${transformations.width}`);
    if (transformations.height)
      params.append("tr", `h-${transformations.height}`);
    if (transformations.quality)
      params.append("tr", `q-${transformations.quality}`);
    if (transformations.format)
      params.append("tr", `f-${transformations.format}`);
    if (transformations.crop) params.append("tr", `c-${transformations.crop}`);
    if (transformations.focus)
      params.append("tr", `fo-${transformations.focus}`);
  }
